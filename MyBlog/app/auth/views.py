...
def login():
    ...
    if form.validate_on_submit():
        info = UserInfo.query.filter_by(email=form.email.data).first()
        if info is not None:
            uid = info.uid
...
def resend_confirmation():
    ...
    userinfo = UserInfo.query.get(current_user.uid)
    send_email(userinfo.email, 'Confirm Your Account',
               'auth/email/confirm', user=current_user, userinfo=userinfo, token=token)
    flash('A confirmation email has been sent to your email.')
    return redirect(url_for('main.index'))
